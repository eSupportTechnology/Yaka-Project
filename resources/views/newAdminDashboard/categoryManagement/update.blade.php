                    ...
                    <form class="forms-sample" id="admin-form" method="post" action="{{route('dashboard.categories.update-category', $category->url)}}" enctype="multipart/form-data" >
                        ...
                        @if(!isset($maincategory)) 
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" id="count" name="free_add_count" value="{{ old('free_add_count') ?? ($category->mainId != 0 ? 0 : $category->free_add_count) }}" placeholder="Free add count">
                            </div>
                        @endif
