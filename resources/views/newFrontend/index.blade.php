...
@section('content')
    ...
    @media (max-width: 768px) {
        ...
        .superad-card.card-left {
            left: 1rem; transform: scale(0.6) translateX(-10rem) !important;
        }
        ...
        .superad-card.card-right {
           right: 1rem; transform: scale(0.6) translateX(10rem) !important;
        }
    ...
    <div class="container">
        <div class="row justify-content-center text-center">
            ...
            @if(count($superAds) > 0)
                <div class="cards-container" style="position: relative; height: 600px; margin: 60px auto; max-width: 900px; display: flex; justify-content: center; align-items: center;">
                    ...
                    @foreach ($displayAds as $index => $adss)
                        <div class="superad-card card-{{ $index == 0 ? 'left' : ($index == 1 ? 'center' : 'right') }}"
                            ...
                            @if($index == 0)
                                left: 7rem; transform: scale(0.85) translateX(-30px); opacity: 0.7; z-index: 1;
                            @elseif($index == 1)
                                left: 50%; transform: translateX(-50%) scale(1.1); opacity: 1; z-index: 3;
                            @else
                                right: 7rem; transform: scale(0.85) translateX(30px); opacity: 0.7; z-index: 1;
                            @endif">

                            <a href="/browse_ads_details/{{ $adss->adsId }}" style="display: block; height: 100%; text-decoration: none; color: inherit;">
                                <img src="{{ asset('storage/' . $adss->mainImage) }}" class="d-block w-100 h-100 card-image"
                                ...
                                <div style="position: absolute; bottom: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(to top, rgba(0, 0, 0, 0.5), transparent); border-radius: 5px;"></div>
                            </a>
                            ...
                            <div class="p-2 details"
                                ...
                                <p class="card-category" style="margin-bottom: 8px; font-size: 14px;">
                                    {{ $adss->category->name ?? 'Uncategorized' }} &raquo; {{ $adss->subcategory->name ?? '' }}
                                </p>

                                <h3 class="card-title" style="font-weight: bold; font-size: 1.1rem; color: white; margin-bottom: 8px;">
                                    {{ $adss->title }}
                                </h3>

                                <p class="price card-price" style="color: white; font-size: 1.2rem; margin-bottom: 8px;">
                                    @lang('messages.Rs') {{ number_format($adss->price, 2) }}
                                </p>

                                <p class="card-time"><i class="fas fa-clock"></i> {{ $adss->created_at->diffForHumans() }}</p>
                            </div>
                        </div>
                    @endforeach
                    ...
                    @if($totalAds > 1)
                        ...
                        <div class="indicators" style="position: absolute; bottom: -40px; left: 50%; transform: translateX(-50%); display: flex; gap: 8px; z-index: 5;">
                            @foreach ($superAds as $indicatorIndex => $ad)
                                <div class="indicator {{ $indicatorIndex == 0 ? 'active' : '' }}" onclick="goToSlide({{ $indicatorIndex }})"
                                    style="width: 10px; height: 10px; border-radius: 50%; background: {{ $indicatorIndex == 0 ? '#0b128e' : 'rgba(11, 18, 142, 0.3)' }}; cursor: pointer; transition: all 0.3s ease;"></div>
                            @endforeach
                        </div>
                    @endif
                </div>
    ...
    <div class="row auto-container">
        <div class="row justify-content-center text-center">
            ...
            @if(count($topAds) > 0)
                <div class="top-ads-container" style="position: relative; height: 600px; margin: 60px auto; max-width: 900px; display: flex; justify-content: center; align-items: center;">
                    ...
                    @foreach ($displayTopAds as $index => $ad)
                        <div class="topad-card card-{{ $index == 0 ? 'left' : ($index == 1 ? 'center' : 'right') }}"
                            ...
                            <a href="/browse_ads_details/{{ $ad->adsId }}" style="display: block; height: 100%; text-decoration: none; color: inherit;">
                                <img src="{{ asset('storage/' . $ad->mainImage) }}" class="d-block w-100 h-100 top-card-image"
                            ...
                            <div class="p-2 details"
                                ...
                                <p class="top-card-category" style="margin-bottom: 8px; font-size: 14px;">
                                    {{ $ad->category->name ?? 'Uncategorized' }} &raquo; {{ $ad->subcategory->name ?? '' }}
                                </p>

                                <h3 class="top-card-title" style="font-weight: bold; font-size: 1.1rem; color: white; margin-bottom: 8px;">
                                    {{ $ad->title }}
                                </h3>

                                <p class="price top-card-price" style="color: white; font-size: 1.2rem; margin-bottom: 8px;">
                                    @lang('messages.Rs') {{ number_format($ad->price, 2) }}
                                </p>

                                <p class="top-card-time"><i class="fas fa-clock"></i> {{ $ad->created_at->diffForHumans() }}</p>
                            </div>
                        </div>
                    @endforeach
                    ...
                    @if($totalTopAds > 1)
                        ...
                        <div class="top-indicators" style="position: absolute; bottom: -40px; left: 50%; transform: translateX(-50%); display: flex; gap: 8px; z-index: 5;">
                            @foreach ($topAds as $indicatorIndex => $topAd)
                                <div class="top-indicator {{ $indicatorIndex == 0 ? 'active' : '' }}" onclick="goToTopSlide({{ $indicatorIndex }})"
                                    style="width: 10px; height: 10px; border-radius: 50%; background: {{ $indicatorIndex == 0 ? '#00ff44' : 'rgba(0, 255, 68, 0.3)' }}; cursor: pointer; transition: all 0.3s ease;"></div>
                            @endforeach
                        </div>
                    @endif
                </div>
                    ...
                    <script>
                        document.addEventListener('DOMContentLoaded', function() {
                            ...
                            function updateTopCards() {
                                const leftIndex = (currentTopIndex - 1 + totalTopItems) % totalTopItems;
                                const centerIndex = currentTopIndex;
                                const rightIndex = (currentTopIndex + 1) % totalTopItems;

                                const topCards = document.querySelectorAll('.topad-card');
                                if (topCards.length >= 3) {
                                    const leftCard = topCards[0];
                                    const centerCard = topCards[1];
                                    const rightCard = topCards[2];

                                    updateTopCard(leftCard, topAdsData[leftIndex]);
                                    updateTopCard(centerCard, topAdsData[centerIndex]);
                                    updateTopCard(rightCard, topAdsData[rightIndex]);
                                }
                            ...
                            // Auto-play functionality for top ads (optional)
        ...
        @if(count($latestAds) > 0)
            <div class="urgent-ads-container" style="position: relative; margin: 40px auto; overflow: hidden;">
                <div class="urgent-cards-wrapper" style="display: flex; transition: transform 0.5s ease;">
                    ...
                    <div class="large-device-slider" style="display: flex; width: 100%;">
                        @foreach ($chunkedAdsLarge as $groupIndex => $adsGroup)
                            <div class="urgent-cards-group large-group" style="display: flex; min-width: 100%; gap: 20px; justify-content: center; align-items: stretch;">
                                @foreach ($adsGroup as $ads)
                                    <div class="urgent-card" style="flex: 1; max-width: 280px; min-width: 250px;">
                                        <a href="/browse_ads_details/{{ $ads->adsId }}" style="text-decoration: none; color: inherit;">
                                            <div class="feature-block-one">
                                                <div class="inner-box urgent-inner-box"
                                                    ...
                                                    <div class="image-box" style="position: relative; flex-shrink: 0;">
                                                        <figure class="image" style="margin: 0; overflow: hidden;">
                                                            <img src="{{ asset('storage/' . $ads->mainImage) }}"
                                                                 alt="{{ $ads->title }}"
                                                                 class="urgent-card-image"
                                                                 style="width: 100%; height: 220px; object-fit: contain; background-color: white;">
                                                        </figure>

                                                        <div class="feature" style="background-color: rgb(171, 18, 18); position: absolute; top: 10px; left: 10px; color: white; padding: 5px 10px; font-size: 12px; font-weight: bold; border-radius: 3px; z-index: 2;">
                                                            Urgent
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="lower-content urgent-lower-content"
                                                        ...
                                                        <ul class="clearfix info urgent-card-location" style="list-style: none; padding: 0; margin: 0 0 15px 0;">
                                                            <li style="display: flex; align-items: center; font-size: 14px; color: #666;">
                                                        ...
                                                        <div class="lower-box" style="margin-top: auto;">
                                                            <h5 class="urgent-card-price" style="margin: 0; font-size: 18px; font-weight: bold; color: #e74c3c;">
                                                                <span style="font-weight: normal; color: #333;">Price:</span>
                                ...
                                @for ($i = count($adsGroup); $i < 4; $i++)
                                    <div class="urgent-card-placeholder" style="flex: 1; max-width: 280px; min-width: 250px; visibility: hidden;"></div>
                                @endfor
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="small-device-slider" style="display: none; width: 100%;">
                        @foreach ($chunkedAdsSmall as $groupIndex => $adsGroup)
                            <div class="urgent-cards-group small-group" style="display: flex; min-width: 100%; gap: 15px; justify-content: center; align-items: stretch;">
                                @foreach ($adsGroup as $ads)
                                ...
                                @for ($i = count($adsGroup); $i < 2; $i++)
                                    <div class="urgent-card-placeholder" style="flex: 1; max-width: 300px; min-width: 280px; visibility: hidden;"></div>
                                @endfor
                            </div>
                        @endforeach
                    </div>
                ...
                @if(count($chunkedAdsLarge) > 1 || count($chunkedAdsSmall) > 1)
                    <div class="urgent-navigation large-device-navigation" style="position: absolute; top: 50%; transform: translateY(-50%); width: 100%; pointer-events: none;">
                    ...
                    <!-- Indicators -->
                    <div class="urgent-indicators large-indicators" style="display: flex; justify-content: center; margin-top: 30px; gap: 10px;">
                        @foreach ($chunkedAdsLarge as $indicatorIndex => $group)
                            <div class="urgent-indicator {{ $indicatorIndex == 0 ? 'active' : '' }}" onclick="goToUrgentSlide({{ $indicatorIndex }})"
                                style="width: 12px; height: 12px; border-radius: 50%; background: {{ $indicatorIndex == 0 ? '#e74c3c' : 'rgba(231, 76, 60, 0.3)' }}; cursor: pointer; transition: all 0.3s ease;"></div>
                        @endforeach
                    </div>

                    <div class="urgent-indicators small-indicators" style="display: none; justify-content: center; margin-top: 30px; gap: 10px;">
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        function updateUrgentSlider() {
                            ...
                            const wrapper = activeSlider.querySelector('.urgent-cards-group') ? activeSlider : activeSlider.parentElement;
