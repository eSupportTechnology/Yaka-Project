<div class="adpost-card">
    <div class="adpost-title">
        <h3>{{ GoogleTranslate::trans('Electronics -   Electrical Fittings', app()->getLocale())}}</h3>
    </div>
    <br>
    ...
    <br>
        ...
        <div class="col-lg-12">
            <div class="form-group">
                <label class="form-label">{{ GoogleTranslate::trans('Capacity', app()->getLocale())}}</label>
                <select onchange="selecterror('capacity')" class="form-control custom-select" name="capacity">

                @foreach(config('constants.CAPACITY') as $key=>$capacity)
                    <option @if(old('capacity', 'default_value') == $key) selected @endif  value="{{$key}}">{{$capacity}}</option>

                @endforeach
                </select>
                @if ($errors->has('capacity'))
                    <div class="alert alert-danger" id="capacity">
