<form class="product-widget-form" action="{{route('ads.location',[ $nowSearchSubCategoryUrl==0 ? $category->url : $nowSearchSubCategoryUrl, $nowSearchLocation ?? ''])}}" id="filter" method="post">
    ...
    <div class="col-md-6 col-lg-12">
        <div class="product-widget">
            <h6 class="product-widget-title">{{GoogleTranslate::trans('Land size', app()->getLocale())}}</h6>
            <div class="product-widget-group">
                <input type="number" value="{{session('filter_size_of_land_min') ?? 0}}" name="size_of_land_min" placeholder="min - 00">
                <input type="number" value="{{session('filter_size_of_land_max') ?? 0}}" name="size_of_land_max" placeholder="max - 1B">
            </div>
    ...
    <div class="col-md-6 col-lg-12">
        <div class="product-widget">
            <h6 class="product-widget-title">{{GoogleTranslate::trans('House Size', app()->getLocale())}}</h6>
            <div class="product-widget-group">
    ...
    <div class="col-md-6 col-lg-12">
        <div class="product-widget">
            <h6 class="product-widget-title"> {{GoogleTranslate::trans('Bathrooms ', app()->getLocale())}}</h6>
    ...
    <div class="col-md-6 col-lg-12">
        <div class="product-widget">
            <h6 class="product-widget-title">{{GoogleTranslate::trans('Unit', app()->getLocale())}}</h6>
            <ul class="product-widget-list product-widget-scroll">
                @php
                ...
                @foreach(config('constants.UNIT') as $key => $unit)
                    <li class="product-widget-item">
                        <div class="product-widget-checkbox">
                            <input @if(in_array($key, $retrievedArray)) checked @endif  name="unit[]" value="{{ $key }}" type="checkbox" id="">
                        </div>
                        <label class="product-widget-label" for="{">
                            <span class="product-widget-type" style="color: #0D0A0A">{{ $unit }}</span>
                            <span class="product-widget-number"></span>
                        </label>
                    </li>
                @endforeach
