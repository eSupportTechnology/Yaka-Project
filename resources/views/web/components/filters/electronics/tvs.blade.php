<form class="product-widget-form" action="{{route('ads.location',[ $nowSearchSubCategoryUrl==0 ? $category->url : $nowSearchSubCategoryUrl, $nowSearchLocation ?? ''])}}" id="filter" method="post">
    ...
    <div class="col-md-6 col-lg-12">
        <div class="product-widget">
            <h6 class="product-widget-title">{{GoogleTranslate::trans('Item type', app()->getLocale())}}</h6>
            <ul class="product-widget-list product-widget-scroll">
                @php
    ...
    <div class="col-md-6 col-lg-12">
        <div class="product-widget">
            <h6 class="product-widget-title">{{GoogleTranslate::trans('Screen Size', app()->getLocale())}}</h6>
            <ul class="product-widget-list product-widget-scroll">
                @php
                ...
                @foreach(config('constants.SCREEN_SIZE') as $key => $screen_size)
                    <li class="product-widget-item">
                        <div class="product-widget-checkbox">
                            <input @if(in_array($key, $retrievedArray)) checked @endif  name="screen_size[]" value="{{ $key }}" type="checkbox" id="">
                        </div>
                        <label class="product-widget-label" for="{">
                            <span class="product-widget-type" style="color: #0D0A0A">{{ $screen_size }}</span>
                            <span class="product-widget-number"></span>
                        </label>
                    </li>
                @endforeach
